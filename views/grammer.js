...
/*
    서버가 유저에게 실시간으로 정보를 보내고싶으면

    원래 get,post같은 http 요청은 1회요청하면 1회 응답하고 끝난다.
    하지만 1회응답이아니라 지속적으로 서버에ㅓ 응답을 하고싶은경우가있다.
    근데 응답.send()하면 1회응답하고 끝이기 때문에

    1. Header라는 정보의 connection 항목을 keep-alive로 설정해주고
    2. 응답.write('안녕하세요') 이렇게 보내면 계속 유저에게 지속적으로 응답이 가능하다
    ...
    ** Header **
    ...
    Header가 어떻게 생겼는지 보고싶으면 크롬 개발자도구 Network 탭가서 열어보면된다. 
*/
...
/*
    ...
    ** JSON.stringify **
    ...
    { }, [ ] 자료 내부에 전부 따옴표를 붙이고 싶으면 JSON.stringify() 함수 안에 넣어주면된다
    반대로 { }, [ ] 여기에 따옴표 붙였던걸 떼고 싶으면 JSON.parse() 함수 안에 넣어주면됨.
    따옴표를 붙이는 이유
    붙이면 JSON이라는 자료가 되는데 일종의 문자자료 취급을 받기때문

    서버와 실시간 소통할 때는 { } [ ] 이런건 주고받을 수 없고 문자만 주고받을 수있다.
    그래서 저렇게 따옴표 쳐서 JSON으로 만들어서 전송하게 되는것이다
